...
def llm_chat(api_key: str, prompt: str, **kwargs) -> tuple[str, str]:
    """One prompt to Groq, honouring the GROQ_MODEL override when set.

    Returns (answer, model_used) — the model is resolved from what the account
    actually offers, so a retired model ID cannot silently break every AI panel.
    """
    return _llm_chat_raw(api_key, prompt, model=settings.groq_model, **kwargs)
...
Be direct and actionable. No disclaimers."""
                ...
                with st.spinner("Analyzing fundamentals..."):
                    try:
                        answer, used = llm_chat(_groq_key_fund, prompt, max_tokens=1200)
                        st.markdown(answer)
...
Keep it direct and actionable. No disclaimers."""
                        ...
                        with st.spinner("Analyzing options flow..."):
                            try:
                                analysis, _used = llm_chat(_groq_key, prompt, max_tokens=1000)
...
def _llm_error(exc: Exception) -> str:
    ...
    if name == "NotFoundError":
        return "failed: no such model on this Groq account"
    if name in ("AuthenticationError", "PermissionDeniedError"):
        return "failed: Groq rejected the key or blocked the request"
...
def classify_institutions(names: tuple) -> tuple[dict, str]:
    ...
    if not key:
        return {}, "no-key"
    try:
        return classify_with_llm(list(names), key, settings.groq_model), "ok"
...
def institutional_narrative(prompt: str) -> tuple[str, str]:
    ...
    if not key:
        return "", "no-key"
    try:
        text, _used = llm_chat(key, prompt, max_tokens=900)
        return text, "ok"
    except Exception as exc:
        return "", _llm_error(exc)
